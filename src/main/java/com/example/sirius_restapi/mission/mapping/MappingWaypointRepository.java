...
public interface MappingWaypointRepository extends JpaRepository<MappingWayPointEntity,Integer> {
    ...
    @Query("select w from MappingWayPointEntity w join w.mappingMissionEntity m where m.id=:missionId order by w.seq")
    List<MappingWayPointEntity> findAllByMissionId(@Param("missionId") Integer missionId);

    @Query("select w from MappingWayPointEntity w join w.mappingMissionEntity m where m.id=:missionId and w.id=:waypointId")
    ...
    @Query("update MappingWayPointEntity w set w.seq=w.seq+1 " +
            "where w.mappingMissionEntity.id=:mission_id and w.seq>=:seq")
